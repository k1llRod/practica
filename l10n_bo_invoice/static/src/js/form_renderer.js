...
var FormRenderer = BasicRenderer.extend({
    ...
    */
    ...
    _renderTagLabel: function (node) {
        ...
        //INFO
        if(node.attrs.vr_info_message){
            $result.prepend(`
                <div style="margin-right:5px; float:left; position: relative;">
                    <a class="fa fa-info-circle vr_info_link" style="cursor: pointer; color:#008784;" index="0" style=""></a>                    
                    <div class="vr_info_popup" style="display:none;"></div>
                </div>
            `);

            $result.find(".vr_info_link").mouseenter(function(){

                $result.find('.vr_info_popup').css({
                    position:'absolute',
                    top:'30px',
                    left:'0px',
                    backgroundColor:'#68465f',
                    padding:'4px 8px',
                    color:'white',
                    width:'200px',
                    zIndex:9999,
                ...
                }).html(node.attrs.vr_info_message);
                $result.find('.vr_info_popup').stop().fadeIn("fast");
            });
            $result.find(".vr_info_link").mouseleave(function() {
                $result.find('.vr_info_popup').stop().fadeOut("fast");
            });
        }
        //END INFO
